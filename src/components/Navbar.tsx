...
const navLinkClasses =
	"text-white px-3 py-2 rounded-md text-sm font-medium hover:bg-gray-700 hover:text-white no-underline";
const dropdownLinkClasses =
	"block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 no-underline";
				...
				<div className='d-flex align-items-center'>
					{!session ? (
						<Link href='/' passHref className='no-underline'>
							<span className={`nav-link ${navLinkClasses} no-underline`}>
								Home
							</span>
						</Link>
					) : (
						<Link href='/dashboard' passHref className='no-underline'>
							<span className={`nav-link ${navLinkClasses}`}>
								<FaArrowCircleLeft />
							</span>
						</Link>
					)}
